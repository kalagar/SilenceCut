...
def generate_ffmpeg_command(input_file, output_file, silence_intervals, video_duration):
    if silence_intervals:
        ...
        filter_complex = "[0:v]trim=start=0:end={0},setpts=PTS-STARTPTS[v];[0:a]atrim=start=0:end={0},asetpts=PTS-STARTPTS[a]".format(video_duration)

    ffmpeg_command = [
        'ffmpeg', '-i', input_file,
        '-filter_complex', filter_complex,
        '-map', '[v]', '-map', '[a]',
        '-c:v', 'libx264', '-b:v', '19681k', '-preset', 'medium', '-profile:v', 'high',  # Match video encoding settings
        '-c:a', 'aac', '-b:a', '110k', '-ar', '48000',  # Match audio encoding settings
        output_file
    ]
...
def main(input_file, output_file):
    ...
    ffmpeg_command = generate_ffmpeg_command(input_file, output_file, silence_intervals, video_duration)
